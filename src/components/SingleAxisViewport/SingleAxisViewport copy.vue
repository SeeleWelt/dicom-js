...
export default {
    ...
    data(){
        return{
            ...
            fileSet:this.$globalData.fileSet,
    ...
    async mounted(){
        ...
        this.Segmentations = segmentation.state.getSegmentations().map((Segmentation,index)=>{
          return {id:Segmentation.segmentationId,index:index,name:Segmentation.segmentationId}
        })
        console.log("Segmentations",this.Segmentations)
    },
    methods: {
        /**
         * @功能 - 初始化数据
        ...
        exportAsPic()
        {
          ...
          {
            const exportId = viewportInputsObjects[orientation].viewportId
        ...
        handleChange3dPreset(value){
          const viewport3d = getRenderingEngine(renderingEngineId).getViewport(viewportId4)
          if(viewport3d)
        ...
        async handleCreateNewSegmentation() {
          ...
          const newSegmentationId = `分割${this.SegmentationNumber}`;
          const index = this.SegmentationNumber-1
          const curSegmentation = {id:newSegmentationId,index:index,name:newSegmentationId}
        ...
        async renderAnnotationsForDicom()
        {
          ...
          this.$globalData.annotations.forEach(Annotation=>{
            if(contourToolsNames.includes(Annotation.metadata.toolName))
            {
              ...
              const segmentationIndex = parseInt(segmentationId.replace(/\D+/g,""))
        ...
        handleSliceChange(viewportId,value){
          ...
          const viewport = getRenderingEngine(renderingEngineId).getVolumeViewports()
        ...
        handleWindowLevelDropdownCallback(value){
          const windowData = voiData[value]
          const viewports = getRenderingEngine(renderingEngineId).getVolumeViewports()
          if(viewports?.length>0)
          {
            viewports.forEach(viewport=>{
        ...
        handleOrientationDropdownCallback(value){
          ...
          const viewportInput = viewportInputsObjects[value]
          const Id = 'renderZoneItem1'
          this.createViewport(
            Id,
            renderingEngineId,
            viewportInput,
        ...
        async handMultiWindows(name,winodwNumbers)
        {
          ...
          if(name==='single')
          {
            ...
            await this.createViewport(
              id,
              renderingEngineId,
              viewportInputsObjects[this.curOrientation],
              toolGroupId,
              this.fileImageIds,
              segmentationId,
              volumeId,
          ...
          else if(name === 'MPR')
          {
            ...
            const Ids = IdNumbers.map(val=>`renderZoneItem${val}`)
            const viewportInputsObjectsNames = Object.keys(viewportInputsObjects)
            for(let [index,id] of Ids.entries())
            {
              await this.createViewport(
                id,
                renderingEngineId,
                viewportInputsObjects[viewportInputsObjectsNames[index]],
                toolGroupId,
                this.fileImageIds,
                segmentationId,
                volumeId,
            ...
            const volumeViewports = getRenderingEngine(renderingEngineId).getVolumeViewports()
            ...
            this.activeToolsToSpecificToolGroup(toolGroupId,
          ...
          else if(name === '3D')
          {
            ...
            await this.createViewport(
              id,
              renderingEngineId,
              viewportInputsObjects['3D'],
              toolGroup3DId,
              this.fileImageIds,
              segmentationId,
              volume3DId,
            )
            const toolGroup=ToolGroupManager.getToolGroup(toolGroup3DId)
        ...
        async run(){
            ...
            const viewportInput = viewportInputsObjects['轴向']
            const id = "renderZoneItem1"
            await this.createViewport(id,renderingEngineId,viewportInput,toolGroupId,this.fileImageIds,segmentationId,volumeId)
