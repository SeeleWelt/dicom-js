...
export default {
    ...
    watch:{
      original_orientation(newValue){
        this.handleOrientationDropdownCallback(newValue)
      },
      showColorbarsMenus:{
        handler(){
    ...
    async mounted(){
        ...
        // 初始化窗口窗位
        const viewport = getRenderingEngine(this.renderingEngineId).getViewports().find(viewport=>viewport.id.startsWith(this.prefixId))
        ...
        this.$emit("handleOrigianlWL",this.prefixId,this.original_wl)
    },
    methods: {
        /**
         * @功能 - 初始化数据
        ...
        handleWindowLevelDropdownCallback(value){
          ...
          const viewports = getRenderingEngine(this.renderingEngineId).getVolumeViewports().filter(viewport=>viewport.id.startsWith(this.prefixId))
        ...
        viewportReRender(renderingEngineId)
        {
          ...
          if (renderingEngine)
          {
            const viewports = renderingEngine.getViewports().filter(viewport=>viewport.id.startsWith(this.prefixId))
            if(viewports?.length > 0){
              viewports.forEach(viewport=>
              {
                viewport.render()
        ...
        handleSliceChange(viewportId,value){
          ...
          const viewport = getRenderingEngine(this.renderingEngineId).getVolumeViewports().filter(viewport=>viewport.id.startsWith(this.prefixId))
        ...
        setViewportColormap(viewportId,colormapName) {
          ...
          const viewport = (
            renderingEngine.getVolumeViewports().find(viewport=>viewport.id === viewportId)
          );
        ...
        handleChangeSlabThickness(value){
          ...
          const renderingEngine = getRenderingEngine(this.renderingEngineId)
          renderingEngine.getVolumeViewports().filter(viewport=>viewport.id.startsWith(this.prefixId)).
          forEach((viewport)=>{
        ...
        async handleDoubleClickRenderZoneItems(index){
          ...
          .filter(viewport=>viewport.id.startsWith(this.prefixId));
          viewports.forEach(viewport=>{
        ...
        async handMultiWindows(name,winodwNumbers)
        {
          ...
          else if(name === 'MPR')
          {
            ...
            const volumeViewports = getRenderingEngine(this.renderingEngineId).getVolumeViewports().filter(viewport=>viewport.id.startsWith(this.prefixId))
        ...
        )
        {
          ...
          // 先删除之前对应的Viewport
          if(renderingEngine.getViewports().filter(viewport=>viewport.id.startsWith(this.prefixId))?.length>0)
          {
            renderingEngine.getViewports().filter(viewport=>viewport.id.startsWith(this.prefixId)).forEach(viewport=>{
          ...
          if (viewportInput.type === ViewportType.STACK) {
            ...
            this.$emit("handleCurViewportId",viewportId)
